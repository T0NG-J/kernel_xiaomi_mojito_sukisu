...
/*
 * The susfs code is written against the ida_alloc_min()/ida_free() API added
 * in 4.19. Map it onto the ida_simple_* wrappers, which have the same
 * semantics (end == 0 means no upper bound) and do their own locking.
 */
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	// mnt_ids tagged DEFAULT_SUS_MNT_ID_FOR_KSU_PROC_UNSHARE were never
	// handed out by mnt_alloc_id(), so there is nothing to free.
	if (unlikely(mnt->mnt.susfs_mnt_id_backup == DEFAULT_SUS_MNT_ID_FOR_KSU_PROC_UNSHARE))
		return;
	// A sus mnt_id came from susfs_mnt_id_ida, free it there.
	if (unlikely(mnt->mnt_id >= DEFAULT_SUS_MNT_ID)) {
		ida_free(&susfs_mnt_id_ida, mnt->mnt_id);
		return;
	}
	// Otherwise a non-zero backup means mnt_id is spoofed; free the original.
...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
	// A sus mnt_id gets a sus mnt_group_id from susfs_mnt_group_ida.
	if (mnt->mnt_id >= DEFAULT_SUS_MNT_ID) {
		res = ida_alloc_min(&susfs_mnt_group_ida, DEFAULT_SUS_MNT_GROUP_ID,
				    GFP_KERNEL);
		if (res < 0)
			return res;
